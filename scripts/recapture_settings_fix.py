# -*- coding: utf-8 -*-
"""Re-capture Settings tabs reliably, rebuild gradient collage, restore Cursor."""
...
def restore_cursor():
    ...
    def enum_proc(hwnd, _lparam):
        ...
        if pid.value not in pids:
            return True
        if user32.IsIconic(hwnd) or int(hwnd) in restored:
...
def save(img, name):
    ...
    # content check: not nearly-empty dark
    g = img.convert("L").resize((120, 80))
    pix = list(g.getdata())
    mean = sum(pix) / len(pix)
    var = sum((p - mean) ** 2 for p in pix) / len(pix)
    print(f"saved {name} {img.size} mean={mean:.1f} var={var:.1f}")
    return var
...
def capture_settings():
    ...
    time.sleep(1.0)
    # Force layout of every tab twice
    for _ in range(2):
        for _, tab in tabs:
            app.tabview.set(tab)
    ...
    for fname, tab in tabs:
        ...
        var = save(grab_rect(l + 1, t + 1, r - 1, b - 1), fname)
        if var < 120 and tab != "About & Developer":
            # retry once
            app.update()
            time.sleep(0.8)
            l, t, r, b = window_rect(hwnd)
...
def build_collage():
    ...
    draw.text((pad + 1, 91), "Windows 11 TTS Suite  ·  Tray · Reader · Voices · Automation  ·  2026", fill=(0, 0, 0), font=fs)
    draw.text((pad, 90), "Windows 11 TTS Suite  ·  Tray · Reader · Voices · Automation  ·  2026", fill=(235, 240, 255), font=fs)
